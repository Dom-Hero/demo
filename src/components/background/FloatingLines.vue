...
type FloatingLinesProps = {
  ...
  /** canvas: 仅背景区域响应；parent: 父容器范围内响应且背景不拦截点击 */
  interactionScope?: 'canvas' | 'parent';
};
...
function getPropsSnapshot() {
  return JSON.stringify({
    linesGradient: props.linesGradient,
    enabledWaves: props.enabledWaves,
    lineCount: props.lineCount,
    lineDistance: props.lineDistance,
    topWavePosition: props.topWavePosition,
    middleWavePosition: props.middleWavePosition,
    bottomWavePosition: props.bottomWavePosition,
    ...
    interactionScope: props.interactionScope
  });
}
...
const setup = () => {
  ...
  if (props.interactionScope === 'parent') {
  ...
  if (props.linesGradient && props.linesGradient.length > 0) {
    ...
    stops.forEach((hex, i) => {
      ...
      const gradientStop = uniforms.lineGradient.value[i];
      if (gradientStop) {
        gradientStop.set(color.x, color.y, color.z);
      }
    });
  }
  ...
  const boundsElement = containerRef.value;
  const interactionElement =
    props.interactionScope === 'parent'
      ? containerRef.value.parentElement ?? containerRef.value
      : containerRef.value;
  ...
  const handlePointerLeave = () => {
    ...
    if (props.parallax) {
      targetParallaxRef.value.set(0, 0);
    }
  };
...
watch(getPropsSnapshot, (next, prev) => {
  if (!isReady || next === prev) return;
  cleanup?.();
  setup();
});
</script>
...
<template>
  <div
    ...
    :class="{ 'floating-lines-container--pass-through': interactionScope === 'parent' }"
    :style="{
...
.floating-lines-container--pass-through {
  pointer-events: none;
}
</style>
